...
export const EnhancedDateSelector = ({className, isLocked, mutation, update, selected, disabled, getInput = _getInput, ...mutationProps}) => {
    return (
        <Mutation mutation={mutation} update={update} onError={showError} {...mutationProps}>
            {(onChange, m_props) => {
                ...
                return (
                    <DatePicker readOnly={isLocked} className={className} disabled={disabled} selected={selected} customInput={<CustomInput />} onChange={saveDate}/>
                )
            }}
        </Mutation>)
}
