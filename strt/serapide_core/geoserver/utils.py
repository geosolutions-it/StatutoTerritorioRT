...
def create_featurestore(name, data, overwrite=False, charset="UTF-8", workspace=None):
    ...
    try:
        cat.create_featurestore(
            name, data, workspace, overwrite=overwrite, charset=charset)
...
def ensure_workspace_exists(ws_name: str):
    workspace = gs_catalog.get_workspace(ws_name)
    if not workspace:
        workspace = gs_catalog.create_workspace(ws_name, f'http://{ws_name}')
    return workspace
