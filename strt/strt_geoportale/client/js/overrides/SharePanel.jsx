...
import ShareSocials from '@mapstore/components/share/ShareSocials';
import ShareLink from '@mapstore/components/share/ShareLink';
import ShareEmbed from '@mapstore/components/share/ShareEmbed';
import ShareApi from '@mapstore/components/share/ShareApi';
import ShareQRCode from '@mapstore/components/share/ShareQRCode';
import {
    Glyphicon,
    Tabs,
    Tab,
...
import { removeQueryFromUrl, getSharedGeostoryUrl, CENTERANDZOOM, BBOX, MARKERANDZOOM, SHARE_TABS } from '@mapstore/utils/ShareUtils';
import { getLonLatFromPoint } from '@mapstore/utils/CoordinatesUtils';
import SwitchPanel from '@mapstore/components/misc/switch/SwitchPanel';
import Editor from '@mapstore/components/data/identify/coordinates/Editor';
import {set} from '@mapstore/utils/ImmutableUtils';
import OverlayTrigger from '@mapstore/components/misc/OverlayTrigger';
import url from 'url';

function mergeQueryHash(shareUrl, originalQuery) {
    const parsedShareUrl = url.parse(shareUrl);
    const hash = parsedShareUrl.hash || '';
    const { search, ...parsedHash } = url.parse(hash.replace('#', ''), true);
    const queryHash = parsedHash.query || {};
    return url.format({
        ...url.parse(shareUrl),
        hash: '#' + url.format({
            ...parsedHash,
...
class SharePanel extends React.Component {
    static propTypes = {
        ...
        removeQueryParameters: PropTypes.bool
    };
    ...
    render() {
        ...
        const originalHash = url.parse(this.props.shareUrl || location.href).hash || '';
        const originalQueryHash = url.parse(originalHash.replace('#', ''), true).query || {};

        const cleanShareUrl = this.getShareUrl() || location.href;
        const mergedShareUrl = this.props.removeQueryParameters ? cleanShareUrl : mergeQueryHash(cleanShareUrl, originalQueryHash);
        const shareUrl = mergedShareUrl;
        let shareEmbeddedUrl = mergedShareUrl;
        ...
        const shareApiUrl = this.props.shareApiUrl || mergedShareUrl || location.href;
